...
这些示例由 `media-tools` 通过 SiliconFlow Kolors 生成；是否免费以服务商当前额度和价格为准。你可以复制提示词重新生成，或直接用它们测试读图。
These examples were generated by `media-tools` through SiliconFlow Kolors; current quotas and pricing are controlled by the provider. Regenerate them or use the images to test image reading.
...
1. **DSH 原生多模态对话**：配置支持图片输入的模型后，把任意图片直接附加到聊天中。
   **Native multimodal chat**: configure an image-capable model, then attach an image to the DSH conversation.

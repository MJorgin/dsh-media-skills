...
- 📎 **貼上即讀取**——在任何工作階段中貼上、拖曳或挑選圖片；免費視覺模型會把它轉成你目前模型能理解的文字。
...
| 功能 | 作用 | 模型 | 費用 |
|---|---|---|---|
| 📎 貼上圖片讀取 | 在**純文字**工作階段中，輸入列會多出「Add image」按鈕（迴紋針）；貼上的圖片會由視覺模型自動描述，並以文字形式交給目前的模型 | Zhipu GLM-4V-Flash | 免費 |
...
驗證：模型選擇器會顯示**智譜 GLM-4V-Flash（視覺）**。如果你的 Harness 版本支援貼上圖片讀取，輸入列也會有 📎 **Add image** 按鈕——在任何工作階段貼上圖片，它就會以文字描述的形式抵達。
...
| 方式 | 做法 | 時機 |
|---|---|---|
| **A. 直接貼上（建議）** | 在任何工作階段中，點擊 📎 按鈕／拖曳／貼上圖片後送出 | 日常圖片問答——不需儲存檔案、不用切換模型 |

...
### *DeepSeek Harness に「目」と「筆」を —— 無料の画像読み取り・生成スキル*
...
| 📎 画像貼り付け自動読み取り | **テキスト専用セッション**（例: deepseek-v4-pro）の入力欄に「画像を追加」ボタン（クリップ）が現れ、貼り付けた画像はビジョンモデルが自動でテキスト説明に変換して現行モデルへ渡す | 智譜 GLM-4V-Flash | 無料 |
...
4. 確認：モデルセレクターに **「智譜 GLM-4V-Flash（視覚）」** が表示されること。本体が貼り付け読み取りに対応していれば、入力欄左下に **📎「画像を追加」ボタン**が表示される。任意のセッションに画像を貼ると、テキスト説明として届く。
...
| 方法 | 使い方 | 向いている場面 |
|---|---|---|
| **A. 直接貼り付け（推奨）** | 任意のセッションでクリップボタン / ドラッグ / ペーストして送信 | 日常的な画像の質問——ファイル保存もモデル切り替えも不要 |

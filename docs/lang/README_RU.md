...
|---|---|---|---|
| 🖼️ Чтение вставленных картинок | В **текстовых** сессиях (например deepseek-v4-pro) в строке ввода появляется кнопка «Добавить изображение» (иконка изображения); вставленные картинки модель зрения автоматически превращает в текстовое описание для текущей модели (**начиная с v0.1.1 по умолчанию DeepSeek-V4-Flash-Vision-Exp**) | v0.1.1: DeepSeek-Vision-Exp · rc.7/8: Zhipu GLM-4V-Flash | GLM бесплатно; DeepSeek списывается с баланса (по умолчанию в v0.1.1) |
| 🧠 Маршрут модели зрения | На ≤ v0.1.1 после установки **автоматически** добавляется «智譜 GLM-4V-Flash（視覚）」 (**v0.1.6**: добавьте один раз в **Настройки → Модели**) — выбирайте её в новом диалоге и обсуждайте изображения | Zhipu GLM-4V-Flash | Бесплатно |
...
| 🎨 `media-tools` | Генерация картинок, иллюстраций, аватаров, фонов и баннеров | SenseNova U1 Fast → SiliconFlow Kolors | Бесплатно, без водяных знаков |

> ⚠️ Честное замечание: чтение вставленных картинок — это возможность **ядра** DeepSeek Harness — нативно в **v0.1.6 / v0.1.1**, в rc.7 / rc.8 через приложенные патчи ядра (приём изображений в `api-proxy`). Этот бандл даёт **навыки** (на ≤ v0.1.1 также маршрут модели); модель зрения работает на любой сборке DSH.
...
2. **Ключи**:
   - **v0.1.6**: при установке ключи не нужны — оба навыка регистрируются во время выполнения. Ключи навыков (`GLM_API_KEY`, `SILICONFLOW_API_KEY`, опционально `GEMINI_API_KEY`) лежат в `~/.dsh/secrets/media-tools.env` (см. [Ключи](#-ключи)); нативные маршруты моделей вы настраиваете сами в **Настройки → Модели**.
   - **на v0.1.1-rc.1+ дополнительные ключи не нужны** — вставка картинок и маршрут зрения работают на существующем `DEEPSEEK_API_KEY` агента. Для бесплатных движков (или rc.7/rc.8): сначала бесплатно получите ключ — зарегистрируйтесь на [open.bigmodel.cn](https://open.bigmodel.cn) → **API Keys** (glm-4v-flash бесплатен). Для генерации создайте ещё один на [siliconflow.cn](https://siliconflow.cn) (Kolors бесплатен). Затем укажите — веб-интерфейс (**Настройки → Модели**) или файл учётных данных:
...
4. Проверьте: в списке навыков появились **`vision-review`** и **`media-tools`** (на v0.1.6 маршрут зрения настраивается в **Настройки → Модели**; на ≤ v0.1.1 в селекторе моделей появится **智譜 GLM-4V-Flash（視覚）**). Если ваша сборка поддерживает вставку картинок, в строке ввода появится кнопка 🖼️ **«Добавить изображение»** — вставьте картинку в любой сессии, и она придёт как текстовое описание.

Полное руководство, принцип работы и решение проблем: **[../SETUP_VISION.md](../SETUP_VISION.md)**

## 🔑 Ключи

Ключи **никогда не хранятся в этом репозитории**. Скрипты читают их по порядку: переменные окружения → `~/.dsh/secrets/media-tools.env` → `~/.codex/secrets/media-tools.env` (устаревший запасной вариант). Маршрут модели зрения читает `GLM_API_KEY` из хранилища учётных данных DSH (≤ v0.1.1; в v0.1.6 маршруты вы настраиваете сами в **Настройки → Модели**).
...
├── index.js               # регистрирует два навыка через жизненный цикл провайдера DSH v0.1.6
├── skills/

...
|---|---|---|
| 粘贴链路 | 本体视觉转述补丁（rc.7/rc.8 挂在 api-proxy，v0.1.2 起迁至 session-controller）：图片附件在进入纯文本模型前由视觉路由转成文字描述（`已由视觉模型读取`），描述直接进消息 | 浏览器端拦截粘贴 → 字节上传 `/modlens/paste` 存临时文件 → **文件路径进输入框** → 模型调 `modlens_read_image` 工具读图；另有 `(modlens vision)` 包装模型变体在请求时转换 |
...
2. **选了纯文本模型**（如 `deepseek-v4-pro`）：
   ...
   - 若 modlens 未接管（裁决为否、路由未就绪、或首次粘贴），图片作为附件正常进入 → **本项目的本体视觉转述补丁接手**，用智谱视觉路由转成文字描述。

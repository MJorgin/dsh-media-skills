...
> ⚠️ **历史归档：DSH v0.1.6 新用户不要应用本文中的补丁。**
>
> 这些资料只用于维护 `dsh-v0.1.0-rc.7`、`rc.8`、`dsh-v0.1.1-rc.1`、`rc.2`
> 及更早的历史构建。DeepSeek Harness v0.1.6 已具备新的附件/文件处理能力，
> 且当前 `dsh-media-skills` 不再隐式写入模型路由。新安装请回到
> [README](../README.md) 或 [v0.1.6 配置指南](SETUP_VISION.md)。

> 下文保留的是当时的技术背景和补丁验证记录，不应当作为 v0.1.6 的安装步骤。
